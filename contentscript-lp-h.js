...
chrome.extension.onMessage.addListener(
  function(request, sender, sendResponse) {
	...
	else if('cannext' == ccmd)
	{
  	...
  	{
  		if(currentUrl.indexOf("showmytalentlist")>0)
		{
...
{
	var tresumeLinks = [];
	$("div.resume-list a.user-name").each(function(index){
		var link = '';
...
{
	var resume=$("div.resume").html();
	var data = "\ufeff"+" <div id='teamface'> <div class='resume'><center> <a href='"+currentUrl+"' target='_blank'>简历原文</a></center>"+resume +"</div></div>";  
	var blob = new Blob([data], { type: 'text/html,charset=UTF-8'});
	var aLink = document.createElement('a');
    aLink.download = "h.liepin.com-"+document.title + ".html";
...
function getResumeLinks2(callback)
{
	...
	getPageNum(pageNum => {
		...
		chrome.storage.local.get('totalPage', function (items) {
			...
			{
				// 我关注的
				clink = "https://h.liepin.com/connection/getmyattentionpagelist.json?curPage="+(pageNum-1);
				$.ajax({
					url: clink,
					cache: false,
					type: "post",
					...
					//sendMsg(url,msg, 'users');
					msg.data.list.map(item => {
						var link = '';
						var href = item.detailUrl;
						var name = item.simpleResumeForm.resEname;
						link=href;
						if(href.indexOf("https")<0)
						{
							link = 'https://h.liepin.com'+$(this).attr("href");
						}
						tresumeLinks.push(link);
						console.log(name+":"+link);
					});
